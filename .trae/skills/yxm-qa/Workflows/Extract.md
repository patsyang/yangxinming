...
| 输入 | 工具 | 注意 |
|------|------|------|
| URL（普通网页） | Codex 网页工具 | 需要登录的页面按当前 Codex 会话可用能力处理 |
| arxiv 链接 | Codex 网页工具（HTML 版优先） | 拿 abstract + 方法 + 实验段 |
| PDF / 本地文件 | Codex 文件/PDF 读取工具 | 大 PDF 分段读 |
| 直接文本 | 跳过 | 直接进入步骤 2 |
| 论文/书名 | Codex 网页搜索工具 | 拿到 URL 再读取正文 |
...
- **形式化**：用文字 + 简单符号（`-> = != + x` 等）把思想压成一行可视的关系——是"思想的几何"，不是"数学的公式"。让读者一眼看出对应关系。详见 `../References/QuestionDesign.md` 的"形式化的写法"
...
New-Item -ItemType Directory -Force output/pat-qa
```

文件名：`{timestamp}-qa-{主题}_qa.md`

- `qa-` 前缀：标记 Q-A 类型（与 pat-paper 的 `paper-` 同构）
- 主题：核心论点的 5-10 字提炼，去标点。优先用方法名/概念名/灵魂句关键字
- `_qa` 后缀：标记 Q-A 类型，便于搜索
