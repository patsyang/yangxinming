---
name: yxm-rank
description: 给一个领域，找出背后真正撑着它的几根独立的力。十几个现象砍到不可再少的生成器——砍完能把现象一个个生回来，才算数。用户说“降秩”“找秩”“秩是什么”“这个领域靠什么撑着”“背后是什么”，或想把任意领域拆到不可再少的生成器时使用。
---
...
7. **预测 + 变更双测**——两道闸一起过:
   - **预测清单外**:这几根能不能推出清单里没有的现象?推出来的,现实里真有吗?
...
**演示要求**:每条箭头要标 `+` 或 `-`;正环(自我加强)和负环(自我抑制)的总效应在图旁边注一句——不写正负的环路图等于没画。
...
1. 获取时间戳：PowerShell 使用 `Get-Date -Format 'yyyyMMddTHHmmss'` 和 `Get-Date -Format 'yyyy-MM-dd ddd HH:mm'`
2. 写入当前项目根目录下的 `output/pat-rank/{时间戳}--{领域}的秩_rank.md`
3. 输出结构参考 `references/template.md`，最终文件必须是 Markdown。
4. 报告文件路径给用户
