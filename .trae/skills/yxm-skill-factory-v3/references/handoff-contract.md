...
防"随意停止"的承重结构。各角色（含鲁班）不靠氛围记忆协作，只靠 `state.md` + handoff。这条契约把"会不会中途停下来问用户"变成一个**可校验的布尔**，而不是祈使。（这是机械机制，可降级——核心是 state 留痕可追溯，handoff 四件的形式可简化。）
...
见 `ten-kings.md` 同名段。要点：判官在 fresh-context 子 agent 里攻断主污染；不支持时诚实声明"同上下文尽力对抗，不具外部独立保证"（独立性降级，非产物运行降级）。执行产物也守这条——spawn 独立子 agent（不是女娲扮演），运行证据必须留真，留假等于自欺。
