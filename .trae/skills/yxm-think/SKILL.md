---
name: yxm-think
description: 追本之箭——纵向深钻思维工具。给一个观点、现象或问题，像箭一样一路向下钻到不可再分的本质。用户说“想透”“追本”“本质是什么”“为什么会这样”“深挖”“钻到底”，或希望把一个想法、现象纵向追到不可再分的根部时使用。用户给出一句陈述并需要深度分析而不是横向综述时也触发。
...
1. 获取时间戳：Windows PowerShell 使用 `Get-Date -Format "yyyyMMddTHHmmss"` 和 `Get-Date -Format "yyyy-MM-dd ddd HH:mm"`
2. 写入 `output/pat-think/{主题}-追本{时间戳}-_think.md`
3. 报告文件路径给用户
