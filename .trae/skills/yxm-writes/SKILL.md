...
- 时间戳：Windows PowerShell 使用 `Get-Date -Format "yyyyMMddTHHmmss"`
- 可读时间：Windows PowerShell 使用 `Get-Date -Format "yyyy-MM-dd ddd HH:mm"`
- 文件名：`{时间戳}-{标题关键词}_write.md`
- 输出路径：`output/pat-writes/{时间戳}-{标题关键词}_write.md`
...
- 能两个字说的不用四个字。「进行讨论」改成「聊」，「实现功能」改成「做到」
- 每个动词是一次判断。「放」「搁」「摆」不是一回事
- 砍：机械连词（「此外」「另外」）、形容词通胀（「非常重要的关键」改成「关键」）、软化词（「某种程度上」「值得注意的是」）
...
总量：1200-1600 字。少于 1200，没挖够；多于 1600，没砍够。
...
1. 初稿 + 中文重写，两稿择优。
2. 用 Windows PowerShell 获取时间戳：

   ```powershell
   $timestamp = Get-Date -Format "yyyyMMddTHHmmss"
   $readableTime = Get-Date -Format "yyyy-MM-dd ddd HH:mm"
   ```
...
4. 确保输出目录存在：
   ...
   ```powershell
   New-Item -ItemType Directory -Force output/pat-writes
   ```

5. 写入 `./output/pat-writes/{时间戳}-{标题关键词}_write.md`。
6. 报告路径。
